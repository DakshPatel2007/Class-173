...
AFRAME.registerComponent("markerhandler", {
  ...
  handleRatings: async function (dish) {
    var tNumber;
    tableNumber <= 9?(tNumber = `T0${tableNumber}`): `T${tNumber}`
    var orderSummary = await this.getOrderSummary(tNumber)
    var currentOrders = Object.keys(orderSummary.current_orders)
    if(currentOrders.length() > 0 && currentOrders === dish.id){
      ...
      saveRatingButton.addEventListener("click",()=>{
        ...
        firebase.firestore().collection("dishes").doc(dish.id).update({last_review:feedback,last_rating:rating})
        .then(()=>{
          swal({
            icon:"success",
      ...
      swal({
        icon:"warning",
        title:"Sorry!",
        text: "No dish found in the ratings",
        timer: 2500,
        buttons:false,
      })
    }
